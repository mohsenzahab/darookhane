...
import 'package:jalali_date_picker/src/utils/util.dart';
...
///    time picker.
///
class JalaliCalendarDatePicker extends StatefulWidget {
  ...
  })  : initialDate = JalaliDateUtils.dateOnly(initialDate),
        firstDate = JalaliDateUtils.dateOnly(firstDate),
        lastDate = JalaliDateUtils.dateOnly(lastDate),
        currentDate = JalaliDateUtils.dateOnly(currentDate ?? DateTime.now()),
        super(key: key) {
    assert(
  ...
  @override
  State<JalaliCalendarDatePicker> createState() =>
      _JalaliCalendarDatePickerState();
}

class _JalaliCalendarDatePickerState extends State<JalaliCalendarDatePicker> {
  ...
  void initState() {
    ...
    _currentDisplayedMonthDate = DateTime(widget.initialDate.year,
        ...
        .monthStartDate(calendarMode);
    _selectedDate = widget.initialDate;
  }

  @override
  void didUpdateWidget(JalaliCalendarDatePicker oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.initialCalendarMode != oldWidget.initialCalendarMode) {
      _mode = widget.initialCalendarMode;
    }
    if (!JalaliDateUtils.isSameDay(widget.initialDate, oldWidget.initialDate)) {
      _currentDisplayedMonthDate = DateTime(widget.initialDate.year,
          widget.initialDate.month, widget.initialDate.day);
      _selectedDate = widget.initialDate;
    }
  ...
  void _handleMonthChanged(DateTime date) {
    setState(() {
      if (_currentDisplayedMonthDate.year != date.year ||
        ...
        _currentDisplayedMonthDate = DateTime(date.year, date.month, date.day);
    ...
    return Stack(
      children: <Widget>[
        ...
        _DatePickerModeToggleButton(
          mode: _mode,
          title: JalaliDateUtils.formatMonthYear(_currentDisplayedMonthDate,
              Localizations.localeOf(context), _localizations, calendarMode),
          onTitlePressed: () {
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  void initState() {
    ...
    _previousMonthDate = _currentMonth.addMonth(-1, calendarMode);
    _nextMonthDate = _currentMonth.addMonth(1, calendarMode);
    _pageController = PageController(
        initialPage: JalaliDateUtils.monthDelta(
            widget.firstDate, _currentMonth, calendarMode));
  ...
  void _handleMonthPageChanged(int monthPage) {
    setState(() {
      final DateTime monthDate = JalaliDateUtils.addMonthsToMonthDate(
          widget.firstDate, monthPage, calendarMode);
      if (!JalaliDateUtils.isSameMonth(
          _currentMonth, monthDate, calendarMode)) {
        _currentMonth =
        ...
        if (_focusedDay != null &&
            !JalaliDateUtils.isSameMonth(
                _focusedDay, _currentMonth, calendarMode)) {
  ...
  DateTime? _focusableDayForMonth(DateTime month, int preferredDay) {
    final int daysInMonth = JalaliDateUtils.getDaysInMonth(month, calendarMode);
  ...
  void _showMonth(DateTime month, {bool jump = false}) {
    final int monthPage =
        JalaliDateUtils.monthDelta(widget.firstDate, month, calendarMode);
    if (jump) {
  ...
  void _handleGridFocusChange(bool focused) {
    setState(() {
      if (focused && _focusedDay == null) {
        if (JalaliDateUtils.isSameMonth(
            widget.selectedDate, _currentMonth, calendarMode)) {
          _focusedDay = widget.selectedDate;
        } else if (JalaliDateUtils.isSameMonth(
            widget.currentDate, _currentMonth, calendarMode)) {
          _focusedDay =
  ...
  void _handleDirectionFocus(DirectionalFocusIntent intent) {
    ...
    setState(() {
      final DateTime? nextDate =
          _nextDateInDirection(_focusedDay!, intent.direction, calendarMode);
      if (nextDate != null) {
  ...
  DateTime? _nextDateInDirection(
      DateTime date, TraversalDirection direction, CalendarMode calendarMode) {
    final TextDirection textDirection = Directionality.of(context);
    DateTime nextDate = JalaliDateUtils.addDaysToDate(
        date, _dayDirectionOffset(direction, textDirection), calendarMode);
    while (!nextDate.isBefore(widget.firstDate) &&
        !nextDate.isAfter(widget.lastDate)) {
      if (_isSelectable(nextDate)) {
        return nextDate;
      }
      nextDate = JalaliDateUtils.addDaysToDate(nextDate,
          _dayDirectionOffset(direction, textDirection), calendarMode);
    }
    return null;
  }
  ...
  Widget _buildItems(BuildContext context, int index) {
    final DateTime month = JalaliDateUtils.addMonthsToMonthDate(
        widget.firstDate, index, calendarMode);
    return _DayPicker(
            ...
            child: FocusableActionDetector(
              ...
              child: _FocusedDate(
                ...
                child: PageView.builder(
                  ...
                  itemCount: JalaliDateUtils.monthDelta(
                          widget.firstDate, widget.lastDate, calendarMode) +
                      1,
...
class _FocusedDate extends InheritedWidget {
  ...
  bool updateShouldNotify(_FocusedDate oldWidget) {
    return !JalaliDateUtils.isSameDay(date, oldWidget.date);
  }
...
class _DayPickerState extends State<_DayPicker> {
  ...
  void initState() {
    ...
    final int daysInMonth =
        JalaliDateUtils.getDaysInMonth(widget.displayedMonth, calendarMode);
  ...
  void didChangeDependencies() {
    ...
    if (focusedDate != null &&
        JalaliDateUtils.isSameMonth(
            widget.displayedMonth, focusedDate, calendarMode)) {
  ...
  List<Widget> _dayHeaders(
    ...
    for (int i = JalaliDateUtils.firstDayOfWeek(calendarMode);
        true;
        i = (i + 1) % 7) {
      ...
      if (i == (JalaliDateUtils.firstDayOfWeek(calendarMode) - 1) % 7) break;
    }
    return result;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final int daysInMonth =
        JalaliDateUtils.getDaysInMonth(widget.displayedMonth, calendarMode);
    final int dayOffset = JalaliDateUtils.firstDayOffset(
        widget.displayedMonth, calendarMode, localizations);
    ...
    final DateTime monthFirstDay =
        widget.displayedMonth.monthStartDate(calendarMode);
    while (day < daysInMonth) {
      day++;
      if (day < 1) {
        dayItems.add(Container());
      } else {
        final DateTime dayToBuild = monthFirstDay.add(Duration(days: day - 1));
        final bool isDisabled = dayToBuild.isAfter(widget.lastDate) ||
            dayToBuild.isBefore(widget.firstDate) ||
            (widget.selectableDayPredicate != null &&
                !widget.selectableDayPredicate!(dayToBuild));
        final bool isSelectedDay =
            JalaliDateUtils.isSameDay(widget.selectedDate, dayToBuild);
        final bool isToday =
            JalaliDateUtils.isSameDay(widget.currentDate, dayToBuild);
...
/// [JalaliCalendarDatePicker], or [showDatePicker] which create full date pickers.
///
/// See also:
///
///  * [JalaliCalendarDatePicker], which provides a Material Design date picker
///    interface.
///
...
class YearPicker extends StatefulWidget {
  ...
  YearPicker({
    ...
    this.calendarMode = CalendarMode.gregorian,
        ...
        currentDate = JalaliDateUtils.dateOnly(currentDate ?? DateTime.now()),
        initialDate = JalaliDateUtils.dateOnly(initialDate ?? selectedDate),
        super(key: key);
...
class _YearPickerState extends State<YearPicker> {
  ...
  Widget _buildYearItem(BuildContext context, int index) {
    ...
    switch (calendarMode) {
      case CalendarMode.gregorian:
        year = widget.firstDate.year + index - offset;
        isCurrentYear = year == widget.currentDate.year;
        isSelected = year == widget.selectedDate.year;
        isDisabled =
            year < widget.firstDate.year || year > widget.lastDate.year;
        break;
      case CalendarMode.jalali:
        int firstDateYear, lastDateYear;
        Jalali d = Jalali.fromDateTime(widget.lastDate);
        lastDateYear = d.year;
        d = Jalali.fromDateTime(widget.firstDate);
        firstDateYear = d.year;
        year = d.year + index - offset;
        d = Jalali.fromDateTime(widget.currentDate);
        isCurrentYear = d.year == year;
        d = Jalali.fromDateTime(widget.selectedDate);
        isSelected = d.year == year;
        isDisabled = year < firstDateYear || year > lastDateYear;
    }
    ...
    Widget yearItem = Center(
      child: Container(
        ...
        child: Center(
          child: Semantics(
            selected: isSelected,
            child: Text(MaterialLocalizations.of(context).formatDecimal(year),
                style: itemStyle),
          ),
        ),
      ),
    ...
    if (isDisabled) {
      ...
      yearItem = InkWell(
        ...
        onTap: () => widget.onChanged(tapDate(year)),
        child: yearItem,
      );
    }
  ...
  DateTime tapDate(int year) {
    switch (calendarMode) {
      case CalendarMode.gregorian:
        return DateTime(year, widget.initialDate.month, 1);
      case CalendarMode.jalali:
        Jalali d = Jalali.fromDateTime(widget.initialDate);
        return Jalali(year, d.month, 1).toDateTime();
    }
  }
  ...
  int get _itemCount {
    switch (calendarMode) {
      ...
      case CalendarMode.jalali:
        Jalali s = Jalali.fromDateTime(widget.firstDate);
        Jalali e = Jalali.fromDateTime(widget.lastDate);
